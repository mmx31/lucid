...
function iil_C_CleanupSeenList(&$conn){
	...
	if ($dirty)
		cache_write($conn->login, $conn->host, "read_list", implode(",",$conn->seen_list), false);
}
...
function iil_C_FetchSeenList(&$conn){
	...
	if (!$conn->sl_fetched){
		$conn->seen_list = explode(",",cache_read($conn->login, $conn->host, "read_list"));
...
function iil_C_FetchHeaders(&$conn, $mailbox, $message_set){
	...
	for ($id=$from_i;(($id<=$to_i)&&($fp));$id++){
		...
		if ($ICL_CAPABILITY["flags"]){
			...
			if (is_array($conn->seen_list) && in_array($uid, $conn->seen_list)){
				$result[$id]->seen = true;
			}
		}
	}
